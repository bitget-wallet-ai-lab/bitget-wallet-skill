...
- `docs/social-wallet.md` — Complete integration guide: per-chain parameters (BTC/ETH/SOL/Tron/Stellar + 16 EVM chains), 3 signing modes for swap flow (gasPayMaster, regular EVM tx, Tron tx), common mistakes table.
- Supports: BTC (Taproot/SegWit/Legacy/PSBT), ETH, SOL (native+SPL+versioned), Tron, Ton, Stellar, and all EVM chains via `evm_custom#` prefix.
