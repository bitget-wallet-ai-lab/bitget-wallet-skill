...
**Purpose:** Runs makeOrder, signs `data.txs`, and calls send — all in one process. Avoids the ~60s makeOrder expiry. Supports EVM (`--private-key`) and Solana (`--private-key-sol`). Auto-detects chain from makeOrder response. Keys are used in memory only, never output.
